...
internal class MultipartHeadersReaderTest
{
    ...
    public void ReadNextHeaderTest()
    {
        ...
        Assert.That(reader.ReadNextHeader(out var header, trimValue: default), Is.True);
        ...
        Assert.That(reader.ReadNextHeader(out header, trimValue: default), Is.True);
        Assert.That(reader.Status, Is.EqualTo(MultipartReadingStatus.Done));
        ...
        Assert.That(reader.ReadNextContentDisposition(out var value), Is.True);
        Assert.That(reader.Status, Is.EqualTo(MultipartReadingStatus.Done));
        Assert.That(span[value].SequenceEqual("form-data; name=\"transform\"; filename=\"Transform - utf8.xsl\""u8), Is.True);

        Assert.That(reader.ReadNextContentType(out value), Is.True);
        Assert.That(reader.Status, Is.EqualTo(MultipartReadingStatus.Done));
        ...
        Assert.That(reader.ReadNextHeader(out header, TrimOptions.Max), Is.True);
        Assert.That(reader.Status, Is.EqualTo(MultipartReadingStatus.Done));
        Assert.That(span[header.Name].SequenceEqual(" Content-Disposition "u8), Is.True);
        Assert.That(span[header.Value].SequenceEqual("form-data; name=transform; filename=\"Transform - utf8.xsl\""u8), Is.True);
        ...
        Assert.That(reader.ReadNextHeader(out header), Is.False);
        Assert.That(reader.Status, Is.EqualTo(MultipartReadingStatus.HeadersNotFound));
        Assert.That(header, Is.EqualTo(default(MultipartHeader)));
    }
    ...
    */
    ...
    private static void ReadNextHeaderInvalid(ReadOnlySpan<byte> span, MultipartReadingStatus status, TrimOptions trimValue = default)
    {
        var reader = new MultipartHeadersReader(span);
        Assert.That(reader.ReadNextHeader(out var section, trimValue), Is.EqualTo(status == MultipartReadingStatus.Done));
        Assert.That(section, Is.EqualTo(default(MultipartHeader)));
        Assert.That(reader.Status, Is.EqualTo(status));

        Assert.That(reader.ReadNextHeader(out section, trimValue), Is.EqualTo(status == MultipartReadingStatus.Done));
        Assert.That(section, Is.EqualTo(default(MultipartHeader)));
        Assert.That(reader.Status, Is.EqualTo(status));
    }
}